...
**Goal:** Make the published White Cup landing feel like one composed site by stabilizing the header, softening scene transitions, composing Events/Locations on a reliable grid, and making the menu carousel loop forever.

**Architecture:** Keep the existing React section and provenance model. Add isolated stylesheet layers for navigation, bridges and scene layout so parallel work does not fight the legacy global cascade. The carousel owns a three-copy native-scroll track and exposes only the middle copy to assistive technology.
...
- [ ] **Step 3: Implement the minimal navigation layer**

Remove profile-specific horizontal transforms from the active shell. Add a bounded paper rail with `max-width: 78rem`, `min-height: 4rem`, `top: 1rem`, `margin-inline: auto`, `background: color-mix(in oklch, var(--paper-light) 91%, transparent)`, and transitions limited to `opacity`, `background-color`, `box-shadow`, and `transform: translateY`. Cap the desktop logo at `3.25rem`; keep mobile at `3rem`. Keep active-link underline and all existing ARIA behavior.
...
Render five items and assert both arrow buttons are enabled at the initial state, ArrowLeft from the first logical item wraps to the last logical item, ArrowRight from the last wraps to the first, and cloned cards have `aria-hidden="true"` plus `tabIndex=-1`.
...
- [ ] **Step 4: Add menu content without provenance drift**

If adding dishes, add separate decorative assets and data entries with `actual menu` fallback pricing. Do not label generated food as documentary photography.
...
- [ ] **Step 3: Implement independent layers**
...
- Create: `docs/evidence/continuity-polish/` screenshots as evidence

- [ ] **Step 1: Review all worker diffs and resolve conflicts**
...
Use the browser viewport capability at `1920x1080`, `1648x912`, `390x844`, `320x568`, `1920x720`, and `1536x720`. Check screenshots, console logs, `scrollWidth === clientWidth`, heading/card/action bounding boxes, menu wrap controls, anchor landing, and reduced-motion state.

- [ ] **Step 4: Request code review**

Dispatch a read-only reviewer against the final diff and fix all P0/P1 findings before staging.

- [ ] **Step 5: Build with `/site/whitecup/` base and deploy**
