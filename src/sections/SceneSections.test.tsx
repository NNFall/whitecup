...
describe('White Cup story scenes', () => {
  ...
  it('matches the supplied menu scene copy and semantic card contract', () => {
    ...
    expect(within(menu).getByText('Кофе и завтраки для спокойного утра.')).toBeInTheDocument()
    ...
    expect(within(menu).getByText(/Иллюстрации блюд\. Актуальное меню и цены/i)).toBeInTheDocument()
  ...
  it('matches the supplied Visit scene copy, semantic cards, and independent layers', () => {
    ...
    expect(visit.querySelector('.visit-scene__intro p')).toHaveTextContent(
      'Ваш уютный уголок в центре Самары. Для кофе перед прогулкой, встречи с друзьями или спокойной паузы между делами.',
    )
  ...
  it('matches the supplied Locations scene while preserving verified visit facts', () => {
    ...
    expect(within(locations).getByRole('article', { name: 'Красноармейская, 17' })).toBeInTheDocument()
    expect(
      within(locations).getByRole('article', {
        name: 'Станкозавод, Куйбышева, 128А',
      }),
    ).toBeInTheDocument()
    ...
    expect(within(articles[1]).getByRole('heading', { level: 3 })).toHaveTextContent(
      'Станкозавод, Куйбышева, 128А',
    )
    expect(within(articles[0]).getByText('Часы работы в картах')).toBeInTheDocument()
    expect(within(articles[1]).getByText('Часы работы в картах')).toBeInTheDocument()
    expect(within(articles[0]).getByRole('link', { name: /уточнить часы работы/i })).toHaveAttribute(
      'href',
      'https://yandex.ru/maps/org/white_cup/19381755919/',
    )
    expect(within(articles[1]).getByRole('link', { name: /уточнить часы работы/i })).toHaveAttribute(
      'href',
      'https://yandex.ru/maps/org/white_cup/193710716150/',
    )
  ...
  it('uses verified contact, location and source links', () => {
    ...
    expect(screen.getAllByRole('link', { name: /красноармейская, 17/i }).length).toBeGreaterThan(0)
    expect(screen.getAllByRole('link', { name: /куйбышева, 128А/i }).length).toBeGreaterThan(0)
  ...
  it('keeps both current address variants and the entrance clarification', () => {
    render(<App />)

    const locations = screen.getByRole('region', { name: /как нас найти/i })
    expect(within(locations).getAllByText(/Красноармейская, 17/i).length).toBeGreaterThan(0)
    expect(within(locations).getAllByText(/Куйбышева, 128А/i).length).toBeGreaterThan(0)
    expect(within(locations).getByText(/вход через двор музея модерна/i)).toBeInTheDocument()
  })
})
